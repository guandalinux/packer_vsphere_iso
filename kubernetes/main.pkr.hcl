...
source "vsphere-iso" "kubernetes" {
  ...
  convert_to_template = var.vm_export_mode == "template" ? true : false

  # Export VM to OVF
  dynamic "content_library_destination" {
    for_each = var.vm_export_mode == "ovf" ? [1] : []
    content {
...
build {
  ...
  provisioner "ansible" {
    ...
    ansible_env_vars = [
      ...
      "VM_SYSTEM_TYPE=${var.vm_system_type}",
      "RKE2_PROVIDER_TYPE=${var.rke2_provider_type}",
      "CLOUD_INIT_DS=[${join(", ", var.cloud_init_datasources)}]"
    ]
  }
}